...
export default function ProfilePage() {
  ...
  const AvatarImage = ({ src, alt, className, style, onError }) => {
    ...
    const defaultAvatar =
      "https://via.placeholder.com/150/6c757d/ffffff?text=User";
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const profileUserId =
          userId || currentUserId || localStorage.getItem("userId") || "1";
        ...
        try {
          ...
          if (followingStoriesRes.ok) {
            ...
            setStats((prev) => ({
              ...prev,
              following: followingStoriesRes.data?.length || 0,
            }));
          }
        } catch (err) {
        ...
        try {
          const readingListsRes = await apiRequest(
            `/users/${profileUserId}/reading-lists`
          );
          ...
          for (const list of lists) {
            try {
              const thumbnails = await apiRequest(
                `/reading-lists/${list.id}/thumbnails`
              );
              thumbnailsMap[list.id] = Array.isArray(thumbnails)
                ? thumbnails
                : [];
            } catch (err) {
              console.log(
                `Error fetching thumbnails for list ${list.id}:`,
                err
              );
        ...
        setFetchError(
          "An error occurred while loading data. Some information may not be displayed."
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleUnfollowStory = async (storyId) => {
    try {
      const response = await apiRequest(`/followed-stories/${storyId}`, {
        method: "DELETE",
      });
      if (response.ok) {
  ...
  const handleRemoveFromList = async (listId, storyId) => {
    try {
      ...
      // Update thumbnails
      const thumbnails = await apiRequest(
        `/reading-lists/${listId}/thumbnails`
      );
  ...
  return (
    <div className="min-vh-100 bg-light">
      ...
      <div
        ...
        style={{
          height: "280px",
          backgroundImage: editData.bannerUrl
            ? `url(${editData.bannerUrl})`
      ...
      <div className="container" style={{ marginTop: "80px" }}>
        <div className="text-center mb-3">
          ...
          <div className="d-flex justify-content-center align-items-center gap-2 mb-3 text-muted">
            <span>
              {stats.works} <span className="fw-normal">Works</span>
            </span>
            <span>|</span>
            <span>
              {readingLists.length}{" "}
              <span className="fw-normal">Reading Lists</span>
            </span>
            <span>|</span>
            <span>
              {stats.followers} <span className="fw-normal">Followers</span>
            </span>
          </div>
        </div>
              ...
              <li className="nav-item">
                <button
                  className={`nav-link border-0 px-4 py-3 ${
                    activeTab === "intro"
                      ? "active border-bottom border-3 text-dark fw-semibold"
                      : "text-muted"
                  }`}
          ...
          <div className="row g-4">
            ...
            <div className="col-md-8">
                  ...
                  {userWorks.length > 0 ? (
                    <>
                      <p className="text-muted small mb-3">
                        {
                          userWorks.filter((s) => s.status === "published")
                            .length
                        }{" "}
                        Published Stories •{" "}
                        {userWorks.filter((s) => s.status === "draft").length}{" "}
                              ...
                              <div className="card-body d-flex gap-3">
                                <img
                                  src={
                                    story.cover_image ||
                                    story.cover_url ||
                                    "/assests/icons/default-cover.png"
                                  }
                                  alt={story.title}
              ...
              {activeTab === "library" && (
                <div>
                  ...
                  {readingLists.length === 0 && (
                    <div className="text-center py-5">
                      <i
                        className="bi bi-bookmark text-muted"
                        style={{ fontSize: "64px" }}
                      ></i>
                      <h5 className="mt-3 mb-2">No reading lists yet</h5>
                      <p className="text-muted">
                        Create your first reading list to organize your stories
                      </p>
                    </div>
                  )}
                </div>
              ...
              {activeTab === "following" && (
                <div className="py-4">
                  <h5 className="mb-3">
                    Following ({followingStories.length})
                  </h5>
                  {followingStories.length > 0 ? (
                    ...
                    <div className="text-center py-5">
                      <i
                        className="bi bi-book-heart text-secondary"
                        style={{ fontSize: "64px" }}
                      ></i>
                      <h5 className="mt-4 mb-2">
                        Not following any stories yet
                      </h5>
                      <p className="text-muted">
            ...
            <div className="modal-content">
              <div className="modal-header border-0">
                <div>
                  ...
                  <small className="text-muted">
                    {listStories.length}{" "}
                    {listStories.length === 1 ? "story" : "stories"}
                  </small>
                </div>
                <button
                  type="button"
